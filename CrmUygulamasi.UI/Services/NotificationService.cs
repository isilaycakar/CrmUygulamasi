...
namespace CrmUygulamasi.UI.Services
{
    ...
    public class NotificationService : INotificationService
    {
        ...
        private ITempDataDictionary TempData => _tempDataDictionaryFactory.GetTempData
            (_httpContextAccessor.HttpContext);
        public IList<NotifyData> GetNotifications()
        {
            return TempData.ContainsKey("temp-notify-list") ?
                JsonConvert.DeserializeObject<IList<NotifyData>>(TempData["temp-notify-list"].
                ToString() ?? string.Empty) : new List<NotifyData>();
        }
